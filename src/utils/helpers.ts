...
export const formatSingleProduct = (fullProduct: FullProduct) => {
  ...
  const images = imagesColor.map((imageColor) => imageColor.toJSON().imagePath);

  const capacityAvailable = capacities.map(
    (capacitySize) => capacitySize.toJSON().capacity,
  );

  const colorsAvailable = colors.map(
    (colorData) => colorData.toJSON().color.title,
  );
...
export const formatMultipleProducts = (products: Product[]) => {
  return products.map((product) => {
    const { category, detailId, ...rest } = product.toJSON();
